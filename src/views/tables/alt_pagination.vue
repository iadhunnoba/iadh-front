                    ...
                    <div class="custom-table">
                        <div class="table-header">
                            <div class="d-flex align-items-center">
                                ...
                                <b-button variant="primary" size="sm" class="ml-3" @click="show_user_modal()">Agregar
                                    Usuario</b-button>
                                <b-button-group size="sm" class="ml-3">
                                    <b-button :variant="view_mode === 'students' ? 'info' : 'outline-info'"
                                        @click="set_view_mode('students')">Estudiantes</b-button>
                                    <b-button :variant="view_mode === 'all' ? 'info' : 'outline-info'"
                                        @click="set_view_mode('all')">Todos</b-button>
                                </b-button-group>
                            </div>
                        ...
                        <b-table ref="basic_table" responsive :items="items" :fields="columns"
                            :per-page="table_option.page_size" :current-page="table_option.current_page"
                            :filter="table_option.search_text" sort-by="name" :show-empty="true"
                            @filtered="on_filtered">
                            <template #cell(salary)="row"> ${{ row.item.salary }} </template>
                            <template #cell(action)="row">
                                <div class="actions text-center">
                                    <router-link :to="'/users/profile/' + row.item.id" class="mr-3"
                                        title="Ver Reportes">
                        ...
                        <b-modal id="userModal" :title="edit_user.id ? 'Editar Usuario' : 'Nuevo Usuario'"
                            @ok="save_user">
                            <b-form>
                                <b-form-group label="Email Institutional">
                                    <b-input v-model="edit_user.username"
                                        placeholder="email@comunidad.unnoba.edu.ar"></b-input>
                                </b-form-group>
                                <b-form-group v-if="!edit_user.id" label="Contraseña">
                                    <b-input type="password" v-model="edit_user.password"
                                        placeholder="Contraseña"></b-input>
                                </b-form-group>
                        ...
                        <div class="table-footer">
                            <div class="dataTables_info"> Mostrando del {{ meta.total_items ? meta.start_index + 1 : 0
                            }} al {{ meta.end_index +
                                    1 }} de {{ meta.total_items }}</div>
                            <div class="paginating-container pagination-solid flex-column align-items-right">
                                <b-pagination v-model="table_option.current_page" :total-rows="table_option.total_rows"
                                    :per-page="table_option.page_size" prev-text="Prev" next-text="Next"
                                    first-text="First" last-text="Last" first-class="first" prev-class="prev"
                                    next-class="next" last-class="last" class="rounded">
                                    <template #first-text>
